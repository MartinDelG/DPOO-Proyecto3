...
public class Reserva {
		...
		if (temp) { 
			...
			if(this.getSeguros()!= null) {
				for (String elemento : this.seguros) {
					tarifaDiaria += (Integer.parseInt(aplicacion.getSeguros().get(elemento).getCostoExtra())*diasNecesidad)* (aplicacion.getCategorias().get(aplicacion.getInventario().get(this.getVehiculoAsignado()).getCategoria()).getValorExtraSeguro());
				}}
			if(this.getConductoresAdicionales()!= null) {
	...
	public void generarFacturaPDF(String horaeEntrega, String Seguros, String conductoresLogIn) {
		FacturaPDF factura = new FacturaPDF(this.ID, this.cliente, this.vehiculoAsignado, this.sedeDevolucion, this.sedeRecogida, horaeEntrega, Seguros, conductoresLogIn, this.montoAPagar);
		try {
			factura.facturar();
